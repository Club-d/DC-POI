...
def getCatGraph(cat_seq, seq, time_list):
    i, x, senders, nodes = 0, [], [], {}
    for p, c in zip(seq, cat_seq):
        if p not in nodes:
            nodes[p] = i
            x.append(c) 
            i += 1
        senders.append(nodes[p])
    ...
    return Data(x=x, edge_index=edge_index, num_nodes = len(nodes), edge_time = time_interv)
...
class GraphData(Dataset):
    def __init__(self, n_user, n_poi, n_cat, seq_data, pos_dict, is_eval=False, tr_dict=None):
    ...
    def __getitem__(self, index):
        if not self.is_eval:
            uid, poi, seq, seq_time, cur_time, _, cat_seq, cat = self.seq_data[index]
            ...
            return uid, poi, neg, seq, seq_graph, (cur_time // 60) % 168,  cat_graph, cat
        else:
            uid, poi, seq, seq_time, cur_time, _, cat_seq, cat = self.seq_data[index]
            ...
            return uid, labels.unsqueeze(0), exclude_mask.bool().unsqueeze(0), \
                seq, seq_graph, (cur_time // 60) % 168, cat_graph, cat


def collate_edge(batch):
    u, p, n, s, s_graph, t, c_graph,c = tuple(zip(*batch))
    ...
    c_graph = Batch.from_data_list(c_graph).to(gol.device)

    return u, p, n, s, s_graph, t, c_graph, c

def collate_eval(batch):
    u, label, exclude_mask, seq, seq_graph, t, c_graph, c = tuple(zip(*batch))
    u = torch.LongTensor(u).to(gol.device)
    s_graph = Batch.from_data_list(seq_graph).to(gol.device)
    t = torch.LongTensor(t).to(gol.device)
    c_graph = Batch.from_data_list(c_graph).to(gol.device)

    return u, torch.cat(label, dim=0), torch.cat(exclude_mask, dim=0), seq, s_graph, t, c_graph, c

def getDatasets(path='/Users/dy/Documents/Python/Diff-POI/data/processed/', dataset='nyc'):
    ...
    trn_ds = GraphData(n_user, n_poi, n_cat, trn_set, trn_dict)
    val_ds = GraphData(n_user, n_poi, n_cat, val_set, val_dict, is_eval=True, tr_dict=trn_dict)
    tst_ds = GraphData(n_user, n_poi, n_cat, tst_set, tst_dict, is_eval=True, tr_dict=trn_dict)
    ...
    pLog(f'{dataset.upper()} loaded. #User: {n_user}, #POI: {n_poi}, POI Category: {n_cat}')
    ...
    return n_user, n_poi, n_cat, (trn_ds, val_ds, tst_ds), geo_graph
