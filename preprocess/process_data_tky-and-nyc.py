...
source_pth = f'~/data/raw/test.csv'
dist_pth = f'~/data/processed/{target_dataset.lower()}'
...
if not os.path.exists(review_pth) or True:
    ...
    review_df = pd.read_csv(source_pth, sep=',', header = 0, names=col_names, encoding='unicode_escape')
    ...
    n_user, n_poi, n_cat = pd.unique(review_df['uid']).shape[0], pd.unique(review_df['poi']).shape[0], pd.unique(review_df['catid']).shape[0]
    ...
    #catname clean
    review_df['catname'] = review_df['catname'].str.lower()
    review_df['catname'] = review_df['catname'].apply(lambda x: re.sub(r'[^a-z\s]', '', x))
    review_df['catname'] = review_df['catname'].apply(lambda x: re.sub(r'\s+', ' ', x))

    review_df = review_df[['uid', 'poi', 'lat', 'lon', 'time', 'date', 'catname']]
    ...
    for uid, line in review_df.groupby('uid'):
        ...
        pos_list, time_list, date_list, cat_list =\
            line['poi'].tolist(), line['time'].tolist(), line['date'].tolist(), line['catname'].tolist()

        for i in range(1, len(pos_list)):
            location = (line['lat'].iloc[i], line['lon'].iloc[i])
            if time_list[i] < val_piv:
                trn_set.append((uid, pos_list[i], pos_list[: i], time_list[:i], time_list[i], date_list[i], cat_list[:i], cat_list[i]))
            elif val_piv <= time_list[i] < test_piv:
                val_set.append((uid, pos_list[i], pos_list[: i], time_list[:i], time_list[i], date_list[i], cat_list[:i], cat_list[i]))
            else:
                tst_set.append((uid, pos_list[i], pos_list[: i], time_list[:i], time_list[i], date_list[i], cat_list[:i], cat_list[i]))
    ...
    with open(review_pth, 'wb') as f:
        ...
        pkl.dump((n_cat), f, pkl.HIGHEST_PROTOCOL)
...
print(f'Full data size: {review_df.shape[0]}, #User: {n_user}, #POI: {n_poi}, #POI Category: {n_cat}')
